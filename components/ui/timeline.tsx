...
export const Timeline = ({ data }: { data: TimelineEntry[] }) => {
  ...
  return (
    <div className="w-full rounded-xl md:px-10" ref={containerRef}>
      <div ref={ref} className="relative mx-auto max-w-7xl pb-20">
        {data.map((item, index) => (
          ...
          >
            <div className="sticky top-40 z-10 flex max-w-xs flex-col items-center self-start md:w-full md:flex-row lg:max-w-sm">
              <div className="absolute left-3 flex h-10 w-10 items-center justify-center rounded-full dark:bg-black md:left-3">
              ...
              <h3 className="hidden text-xl font-bold text-primary/80 dark:text-neutral-500 md:block md:pl-20 md:text-3xl">
                {item.title}
              </h3>
            </div>
        ...
        >
          <motion.div
            ...
            className="absolute inset-x-0 top-0 w-[2px] rounded-full bg-gradient-to-t from-neutral-500 from-[0%] to-primary"
          />
        </div>
      </div>
    </div>
